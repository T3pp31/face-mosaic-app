...
各行の16値: `[y_min, x_min, y_max, x_max, kp1_y, kp1_x, ..., kp6_y, kp6_x]`
- 正規化座標 (0-1)
- NMSはモデル内蔵
- 未使用スロットは全0
...
postprocessDetections()
  - 全0行をスキップ
  - [y_min, x_min, y_max, x_max] → [x1, y1, x2, y2] (ピクセル座標)
  - MIN_FACE_SIZE 未満を除外
  ↓
drawImageWithMosaic()
  - 元画像を Canvas に描画
...
| bbox の座標順が y,x | `[y_min, x_min, y_max, x_max]` corner format でパース |
